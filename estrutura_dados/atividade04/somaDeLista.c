...
int somaDeLista(struct NO* lista) {
    //checa se a lista está vazia
    if (lista == NULL){
      printf("A lista está vazia.\n");
      return -1; // retorna um valor especial indicando que a lista está vazia
    } 
    int guardaSoma = 0; // guarda a soma dos valores
    int somaLista = lista->dado; // inicializa o menor valor com o dado do primeiro nó
    ...
    while(ponteiro_atual != NULL){
      guardaSoma = guardaSoma + ponteiro_atual->dado;
      ponteiro_atual = ponteiro_atual->prox; // avança para o próximo nó
      }
      
      return guardaSoma; // retorna a soma dos valores encontrados
}
...
int main() {
    ...
    int resultado = somaDeLista(lista);
